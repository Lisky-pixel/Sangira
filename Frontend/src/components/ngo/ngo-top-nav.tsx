...
const navItems: NgoNavItem[] = [
  { label: ngoPortalContent.topNav.dashboard, to: ROUTES.NGO_DASHBOARD },
  { label: ngoPortalContent.topNav.browse, to: ROUTES.NGO_BROWSE },
  { label: ngoPortalContent.topNav.myRequests, to: ROUTES.NGO_REQUESTS },
  { label: ngoPortalContent.topNav.capacity, to: ROUTES.NGO_CAPACITY },
]

function isNavActive(pathname: string, to: string) {
  if (to === ROUTES.NGO_DASHBOARD) {
    return pathname === ROUTES.NGO_DASHBOARD
  }
...
function NavLinks({
  ...
  return (
    <ul
      className={cn(
        'flex flex-col gap-1 lg:flex-row lg:items-center lg:gap-6',
        className,
      )}
    >
      {navItems.map((item) => {
        ...
        return (
          <li key={item.to}>
            <Link
              ...
              onClick={onNavigate}
              className={cn(
                'block rounded-md px-2 py-2 text-sm font-medium transition-colors lg:px-0 lg:py-1',
                active
                  ? 'text-primary'
...
export function NgoTopNav({ className }: NgoTopNavProps) {
  const { state } = useAuth()
  ...
  const role = isUserRole(state.user.role) ? state.user.role : 'ngo'
  const avatarUrl =
    typeof state.user.avatarUrl === 'string' ? state.user.avatarUrl : undefined
  ...
  return (
    <header className={cn('border-b border-border bg-white', className)}>
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3 sm:px-6 lg:px-8">
          ...
          <Dialog.Root open={mobileOpen} onOpenChange={setMobileOpen}>
            <Dialog.Trigger asChild>
              <button
                ...
                aria-label={ngoPortalContent.topNav.openMenuAria}
                aria-expanded={mobileOpen}
                aria-controls="ngo-mobile-nav"
              >
              ...
              >
                <div className="flex items-center justify-between">
                  <Dialog.Title className="text-charcoal font-display text-lg font-bold">
                    {ngoPortalContent.topNav.brand}
                  </Dialog.Title>
                  <Dialog.Close asChild>
                    <button
                      type="button"
                      className="text-charcoal hover:text-primary"
                      aria-label={ngoPortalContent.topNav.closeMenuAria}
                    >
                ...
                <Dialog.Description className="sr-only">
                  {ngoPortalContent.topNav.mobileNavDescription}
                </Dialog.Description>
                <NavLinks
        ...
        <nav
          aria-label={ngoPortalContent.topNav.navAria}
        ...
        <div className="flex shrink-0 items-center gap-2 sm:gap-3">
          ...
          <div className="hidden items-center gap-2 sm:flex">
            ...
            <VerifiedBadge label={ngoPortalContent.topNav.verifiedNgo} />
          </div>
